.PHONY: help preflight up run-prowler run-cloudquery run-custodian run-steampipe run-billing-export run-billing-ce run-all down clean prepare-dirs
...
prepare-dirs: ## Cria a estrutura de diretórios necessária para os relatórios
	@mkdir -p reports/preflight reports/prowler reports/cloudquery reports/cloud-custodian reports/steampipe reports/billing-native reports/billing-focus reports/billing-cost-explorer logs
	@chmod 777 reports reports/preflight reports/prowler reports/cloudquery reports/cloud-custodian reports/steampipe reports/billing-native reports/billing-focus reports/billing-cost-explorer logs 2>/dev/null || true
...
run-billing-ce: prepare-dirs ## Extrai histórico detalhado de custos dos últimos 90 dias via AWS Cost Explorer
	docker compose run billing-ce
...
run-all: prepare-dirs preflight ## Executa a suíte completa de auditoria em ordem sequencial
	...
	@echo "👉 [2/6] CloudQuery..."
	-docker compose run cloudquery
	@echo "👉 [3/6] Cloud Custodian..."
	-docker compose run custodian
	@echo "👉 [4/6] Steampipe..."
	-docker compose run steampipe
	@echo "👉 [5/6] Billing Export..."
	-docker compose run billing-export
	@echo "👉 [6/6] Billing Cost Explorer..."
	-docker compose run billing-ce
